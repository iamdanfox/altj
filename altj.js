...
var AUGMENT_PROPORTION, App, Graph, GrowComp, NormalDistribution, RaphaelComp, SidebarComp, SpikynessComp, TitleComp, TriModal, a, augment, button, dist, distbetween, div, drawgraph, edgelen, eq, graph, h, h1, input, intersects, intriangle, keypress, label, newpoints, normalDist, randX, randY, setMoreRandom, setMoreRegular, shortcut, triModalDist, w, _ref,
...
randX = function() {
  return w / 2 + Math.random() * 100 - 50;
};

randY = function() {
  return h / 3 + Math.random() * 100 - 50;
};

graph = new Graph([randX(), randY()], [randX(), randY()], [randX(), randY()]);

normalDist = NormalDistribution.make(edgelen(graph.edges[0]), edgelen(graph.edges[1]), edgelen(graph.edges[2]));

triModalDist = TriModal.make(edgelen(graph.edges[0]), edgelen(graph.edges[1]), edgelen(graph.edges[2]));

dist = normalDist;

_ref = React.DOM, h1 = _ref.h1, a = _ref.a, div = _ref.div, button = _ref.button, input = _ref.input, label = _ref.label;
...
GrowComp = React.createClass({
  grow: function() {
    ...
    console.log('grow()');
    console.debug(this);
    if (Math.random() > this.props.augmentProportion) {
      success = shortcut();
      if (!success) {
        return augment();
      }
...
SidebarComp = React.createClass({
  render: function() {
    return div({
      className: 'sidebar'
    }, [TitleComp(), GrowComp(this.props), SpikynessComp(this.props)]);
  }
});
...
drawgraph = function() {
  var b, bucketsize, edge, histogram, key, l, offset, val, x1, x2, y1, y2, _i, _j, _len, _len1, _ref1, _ref2, _ref3, _ref4, _ref5, _results;
  paper.clear();
...
setMoreRandom = function() {
  return dist = normalDist;
};

setMoreRegular = function() {
  return dist = triModalDist;
};
...
augment = function() {
  ...
  if (nps.length > 0) {
    ...
    safeToAdd = function(testpoint) {
      var b, c, p, _i, _j, _k, _len, _len1, _len2, _ref1, _ref2, _ref3, _ref4;
      _ref1 = graph.points;
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        p = _ref1[_i];
